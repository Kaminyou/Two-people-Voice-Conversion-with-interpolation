...
class Decoder(nn.Module):
    def __init__(self, c_in=512, c_out=513, c_h=512, c_a=2, emb_size=128, ns=0.2):
    ...
    def mean_embedding(self, emb1, emb2):
        return torch.mean(torch.cat((emb1, emb2), dim=0), 0, True)

    def forward(self, x, c):
        ##FOR interpolation###
        c1 = Variable(torch.from_numpy(np.array([0]))).cuda()
        c2 = Variable(torch.from_numpy(np.array([1]))).cuda()
        #c3 = torch.mean(torch.cat((self.emb1(c1), self.emb1(c2)), dim=0), 0, True)
        #print(c3)
        #print("c3:", c3.shape)
        # conv layer
        out = self.conv_block(x, [self.conv1, self.conv2], self.ins_norm1, self.mean_embedding(self.emb1(c1), self.emb1(c2)), res=True )
        out = self.conv_block(out, [self.conv3, self.conv4], self.ins_norm2, self.mean_embedding(self.emb2(c1), self.emb2(c2)), res=True)
        out = self.conv_block(out, [self.conv5, self.conv6], self.ins_norm3, self.mean_embedding(self.emb3(c1), self.emb3(c2)), res=True)
        # dense layer
        out = self.dense_block(out, self.mean_embedding(self.emb4(c1), self.emb4(c2)), [self.dense1, self.dense2], self.ins_norm4, res=True)
        out = self.dense_block(out, self.mean_embedding(self.emb4(c1), self.emb4(c2)), [self.dense3, self.dense4], self.ins_norm5, res=True)
        emb = self.mean_embedding(self.emb5(c1), self.emb5(c2))
        out_add = out + emb.view(emb.size(0), emb.size(1), 1)
        # rnn layer
        out_rnn = RNN(out_add, self.RNN)
        out = torch.cat([out, out_rnn], dim=1)
        out = append_emb(self.mean_embedding(self.emb5(c1), self.emb5(c2)), out.size(2), out)
...
if __name__ == '__main__':
    ...
    print(e1.size())
    s1 = S(e1)
    c = Variable(torch.from_numpy(np.random.randint(2, size=(16)))).cuda()
    d = D(e1, c)
    print(d.size())
